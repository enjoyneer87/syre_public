...
currentFolder = pwd;
currentFolder = [currentFolder '\syreExport\syre_AnsysMaxwell'];
...
% %% Savings Variabiles 
% n_pontR           = nnz(geo.pontR);                        % number of radial gap (split ribs considered after)
...
% 
% fclose('all');
% pyfile = fopen( 'setup_draw_motor_in_ansys.txt', 'wt' );
% 
...
% runpy={'sys.exit()'};
% str=sprintf('\n%s\n',runpy{:});
% fprintf(pyfile,str,currentFolder);
% clear runpy
% 
% fclose(pyfile);
% clear pyfile
...
% fclose("all");
% 
% pyfile= fopen('close_projectAM.txt', 'wt');
% 
% closer = {
%     'import sys'
%     'sys.path.append(r"C:/Program Files/AnsysEM/AnsysEM20.1/Win64")'
...
% fclose(pyfile);
% 
% movefile('close_projectAM.txt','close_projectAM.py','f');
% 
% %Ansys start e py run
% command=strcat(ipypath,strrep(currentFolder,'/','\'),'\setup_draw_motor_in_ansys.py"');
% 
% [status,cmdout] = dos(command);
% 
% if status ~= 0
%     disp('an error occurred - programm aborted - \n %s',cmdout)
% end
% command=strcat(ipypath,strrep(currentFolder,'/','\'),'\draw_motor_in_ansys.py"');
% [status,cmdout] = dos(command);
% 
% if status ~= 0
%     disp('an error occurred - programm aborted - \n %s',cmdout)
...
Rotor_MatName=mat.Rotor.MatName; %string's rotor material
Stator_MatName=mat.Stator.MatName; %string's stator material
Shaft_MatName=mat.Shaft.MatName; %string's shaft material
SlotCond_MatName=mat.SlotCond.MatName; %string's Slot Conductor material
LayerMag_MatName=mat.LayerMag.MatName; % string's Magnet material
LayerAir_MatName=mat.LayerAir.MatName; %string's Air material

LayerMag_Hc=mat.LayerMag.Hc; %Magnet's Hc
LayerMag_mu=mat.LayerMag.mu; %Magnet's permeability
LayerMag_sigmaPM=mat.LayerMag.sigmaPM; %magnet's conductivity
LayerMag_kgm3=mat.LayerMag.kgm3; %magnet's density


SlotCond_sigma=mat.SlotCond.sigma; %cond's conductivity
SlotCond_alpha=mat.SlotCond.alpha; %cond's thermal coefficient
SlotCond_kgm3=mat.SlotCond.kgm3; %cond density
...
%%%%%%%%%%%%%% concatenated script's run inside py script %%%%%%%%%%%%%%%%%
%runpy={'execfile("%s/ansys.py")'};
